...
class DaemonThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def _adjust_thread_count(self) -> None:
        ...
        if num_threads < self._max_workers:
            ...
            # right profile (#54937). 3.14 dropped the (initializer, initargs) worker arguments in favour of
            # a per-thread WorkerContext from _create_worker_context(), so pick the shape the running
            # stdlib expects.
